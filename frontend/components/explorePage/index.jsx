  ...
  return (
    <CirclingLightsBackground>
      ...
      <div className={styles.explore}>
        {featuredEvents.length > 0 && featuredEvents[activeIndex] && (
              ...
              <div className={styles.pagination}>
                {featuredEvents.map((event, index) => (
                  <span
                    key={index}
                    className={
                      index === activeIndex ? styles.active : styles.dot
                    }
        ...
        <div className={styles.filters}>
          <div className={styles.dropdown}>
            <div className={styles.dropdownToggle}>
              {/*onClick={toggleDropdown ALSO CHANGE THIS TO A BUTTON WHEN YOU WANT TO IMPLEMENT FEATURED OR OTHER EVENT TYPES*/}
              <div className={styles.filter_events_text}>
                {selectedEventFilter}
              </div>
            </div>
            {isDropdownOpen && (
              <ul className={styles.dropdownMenu}>
                {eventFilters.map((option) => (
                  <li
                    ...
                    onClick={() => handleEventFilterClick(option)}
                  >
                    {option}
                  </li>
                ))}
        ...
        {events.length === 0 && <h1 className="text-neutral-200 font-normal text-lg text-center">None yet. <Link href={LINKS.Discord} className="underline text-blue-500 hover:underline hover:text-blue-600">Join the Discord</Link> to stay updated!</h1>}
